...
该包属于发布产物，不提交到源码分支；发布工作流会把 `.tgz` 和 `index.yaml` 上传为 GitHub Release assets。
...
- `main`: 保存 Chart 源码
- release tag `helm-repo`: 保存 `index.yaml` 和 Chart 包
...
4. 将 `index.yaml` 和 `testudo-chart-1.0.0.tgz` 上传到 `helm-repo` release
...
将 `/tmp/testudo-chart-release/index.yaml` 和 Chart 包上传到 `helm-repo` release 后，即可通过 `helm repo add` 使用。
