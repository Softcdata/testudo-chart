...
- Chart 会固定创建名为 `default-secret` 的 `kubernetes.io/dockerconfigjson` Secret
- `disaster-operator`、`disaster-server`、`disaster-web` 三个 Deployment 都固定引用这个 Secret
- 安装或升级时如果 `imagePullSecret.username` / `imagePullSecret.password` 为空，Helm 渲染会直接失败
...
当前 Chart 会固定创建并引用名为 `default-secret` 的镜像拉取 Secret。你需要在 [values.yaml](/home/chenxi/YS/disaster-system-chart/values.yaml) 中配置 `imagePullSecret.registry`、`imagePullSecret.username` 和 `imagePullSecret.password`。
...
说明：即使是离线导入镜像的场景，当前 Chart 仍然会创建 `default-secret`，因此安装时依然需要提供 `imagePullSecret` 配置。
